...
class NewWorld:
    ...
    def __init__(self, def_obj, obj_size=41, height=240, width=360,
        ...
        self.myMaze = maze_gen.NewMap(m=height, n=width,
                                      n_obstacles=num_obstacles, t_wall=thickness_wall)
        top_left = [self.object_list[0].pos[0]-self.object_list[0].size//2-1,
                    self.object_list[0].pos[1]-self.object_list[0].size//2-1]
        curr_occupancy = self.myMaze.map[top_left[0]:top_left[0]+self.object_list[0].size,
                                         top_left[1]:top_left[1]+self.object_list[0].size]
        while np.sum(curr_occupancy) != self.sum_empty:
            self.myMaze = maze_gen.NewMap(m=height, n=width, n_obstacles=num_obstacles,
                                          t_wall=thickness_wall)
            curr_occupancy = self.myMaze.map[top_left[0]:top_left[0]+self.object_list[0].size,
                                             top_left[1]:top_left[1]+self.object_list[0].size]
    ...
    def __update_collisions(self):
        for obj in self.object_list:
            # Bottom
            if np.sum(self.myMaze.map[obj.pos[0]+obj.size//2+obj.velo[0],
                                      obj.pos[1]-obj.size//2-1:obj.pos[1]+obj.size//2]) < self.sum_edge:
                obj.velo[0] = -obj.velo[0]
            # Right
            if np.sum(self.myMaze.map[obj.pos[0]-obj.size//2-1:obj.pos[0]+obj.size//2,
                                      obj.pos[1]+obj.size//2+obj.velo[1]]) < self.sum_edge:
                obj.velo[1] = -obj.velo[1]
            # Top
            if np.sum(self.myMaze.map[obj.pos[0]-obj.size//2-1+obj.velo[0],
                                      obj.pos[1]-obj.size//2-1:obj.pos[1]+obj.size//2]) < self.sum_edge:
                obj.velo[0] = -obj.velo[0]
            # Left
            if np.sum(self.myMaze.map[obj.pos[0]-obj.size//2-1:obj.pos[0]+obj.size//2,
                                      obj.pos[1]-obj.size//2-1+obj.velo[1]]) < self.sum_edge:
                obj.velo[1] = -obj.velo[1]
